...
Matrix2cd greens(Matrix2cd &OM, double t)
{
	Matrix4cd X,O;
	X << 	0,	0,	1/t,	0,
		0,	0,	0,	1/t,
		-t,	0,	OM(0,0)/t,OM(0,1)/t,
		0,	-t,	OM(1,0)/t,OM(1,1)/t;
...
//initialise surface Green's function using mobius transformation for rotated layer at theta = 0
	...
	Matrix2cd OMV3=params.E*I-V3_0-2.*T_3*F;
	Matrix2cd GR_0;
	GR_0 = greens(OMV3, t_3);
...
//initialise surface Green's function using mobius transformation for rotated layer at theta = PI 
	...
	Matrix2cd OMV2=params.E*I-V3_PI-2.*T_3*F;
	Matrix2cd GR_PI;
	GR_PI = greens(OMV2, t_3);

//initialise surface Green's function using mobius transformation for fixed layer 
	Matrix2cd OMV1=params.E*I-V1-2.*T_1*F;
	Matrix2cd GL;
	GL = greens(OMV1, t_1);

	Matrix2cd OM = params.E*I - V2 - 2.*T_2*F;
	Matrix2cd root_arg = OM*T_2.inverse()*OM*T_2.inverse() - 4.*I;
	Matrix2cd root;
	root <<	sqrt(root_arg(0,0)),	0,
			0,	sqrt(root_arg(1,1));
	Matrix2cd X_p = 0.5*(OM*T_2.inverse() + root);
	Matrix2cd X_m = 0.5*(OM*T_2.inverse() - root);
	...
	Matrix2cd GN = T_2.inverse()*delta_1.inverse()*(delta_N - delta_Nm1*T_2*GL)*(delta_Np1 - delta_N*T_2*GL).inverse()*delta_1;

	Matrix2cd Rsigma_0, Rsigma_PI;
	dcomp Fsigma;
	Rsigma_0 = (I-GR_0*T_2.adjoint()*GN*T_2);
	Rsigma_PI = (I-GR_PI*T_2.adjoint()*GN*T_2);
...
//why doesn't dsi_dE contribute to spincurrent?
	...
	for (double j = 1; j<=N; j = j + 0.1){
		...
		Myfile<< j-1 <<" , "<< -(params.kT/(2.*j))*spincurrent << endl;
